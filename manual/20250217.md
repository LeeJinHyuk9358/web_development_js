...
이상의 두 타입은 속성과 함수가 모두 다르기 때문에 실수로 헷갈리면 에러가 날 수 이 있어서 변수명만 확인하고도 제이쿼리 객체임을 알 수 있도록 '$'를 붙여주는 것이 관례입니다.
...
function showText(s) {
  ...
  $textElement.fadeOut(s);
  $textElement.html(s);
  $textElement.fadeIn(s);
}
```
...
그리고 두 번쨰 argument는 본래 함수인 fadeOut()이 전부 실행된 이후에 .html()과 .fadeIn()이 실행되도록 두 개의 실행문을 하나의 익명함수로 묶은 것을 의미함.
...
4. .prop() : element의 속성(property)을 가져오거나 설정할 떄 사용하는 메서드
