...
- 모든 element를 탐색하기 때문에 느리다고 생각하실 수도 있는데, 브라우저는 미리 id 값과 element를 연결시키는 매핑 테이블(mapping table)을 미리 만들어두기 때문에 document.getElementById()는 매우 빠르고, 다른 DOM element를 탐색하는 모든 함수들 중에서 가장 빠름. -> 최대한 이 함수를 사용하는 것이 성능면에서 바람직 : springboot에서 계속 예를 중점적으로 썼습니다.
...
JS에서는 어떠한 변수를 찾을 때, 현재의 로컬 스코프에서 차장본 후에 없으면 글로벌 스코프가 나올 때까지 하나씩 상위의 부모 스코프로 거슬러 올라가게 됨.
...
그리고 저희가 실제로 이상의 코드까지 구현했을 때 생긴느 불편한 점에 관해 고려할 필요가 있습니다.
...
20250214 수업 시작 -> 남은 슛 횟수가 0이 됐을 때 승패 안내를 하도록 작성할 예정
